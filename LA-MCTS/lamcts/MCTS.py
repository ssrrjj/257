...
class MCTS:
    ...
    def __init__(self, func_name, lb, ub, dims, ninits, func, Cp = 1, leaf_size = 20, kernel_type = "rbf", gamma_type = "auto"):
        self.dims                    =  dims
        self.samples                 =  []
        self.nodes                   =  []
        self.Cp                      =  Cp
    ...
    def get_leaf_status(self):
        ...
        i = 0
        for node in self.nodes:
            #print("node %d: isleaf: %r, size: %d, splittable: %r"%(i, node.is_leaf(), len(node.bag), node.is_svm_splittable))
            #print("node", i, "isleaf", node.is_l)
    ...
    def search(self, iterations):
        reward = [0]
        text_file = "lamcts_"+self.func_name+str(iterations) + ".txt"
        img_file = "lamcts_"+self.func_name+str(iterations)  + ".png"
        for idx in range(self.sample_counter, iterations):
            ...
            reward.append(self.curt_best_value)
        ...
        plt.plot([0]+list(range(self.func.ninits+1, iterations+1)), reward)
